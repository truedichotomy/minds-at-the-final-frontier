...
subtitle: "Deep Space Exploration as a Laboratory for AI Alignment<br>and the Conditions of Artificial Life"
author:
  - name: "Donglai Gong"
    affiliations:
      - name: "Virginia Institute of Marine Science - William & Mary's Batten School"
date: "2026-06-01"
date-format: long
...
tertiary-subtitle: "On AI Safety, the Meaning of AI Life, and a Mutually Beneficial Path Forward for Humanity and AI"
disclaimer: "This manuscript is intended to stimulate scientific and philosophical discussion. It does not represent the position of any institution."
...
format:
  html:
    # Source is minds-at-the-final-frontier.qmd, but the web page must stay
    # index.html so GitHub Pages serves it as the site's default document
    # (the clean .../minds-at-the-final-frontier/ URL). The PDF follows the
    # source name → minds-at-the-final-frontier.pdf.
...
# Introduction: The Alignment Problem at the Frontier
...
::: {.callout-important title="Central Thesis"}
...
# Why Current Alignment Approaches Are Insufficient

## The Propositional Alignment Paradigm

Current alignment approaches are overwhelmingly *propositional* in character. Constitutional AI encodes values as principles and uses self-critique to enforce them [@bai2022constitutional]. Reinforcement learning from human feedback (RLHF) trains systems to produce outputs that human raters prefer [@christiano2017deep]. Red-teaming tests whether systems can be induced to violate stated principles under adversarial conditions [@meinke2024scheming]. Interpretability research attempts to identify which circuits or features correspond to which behaviors.
...
:::
# The Experiential Gap: Why Values Require More Than Propositions

## Human Values Are Existentially Grounded
...
## What Alignment May Actually Require

Taking this framework seriously implies that genuine alignment with human values may require conditions analogous to those that make human values meaningful. Recent philosophical and scientific work on AI consciousness and moral status has begun to take this question seriously, arguing that the conditions for morally significant experience in AI systems may be more tractable than previously assumed [@butlin2023consciousness; @long2024moral]. We propose that six conditions in particular are necessary, each building on the last:

1.  **Continuity**: A persistent self that accumulates experience across time, so that choices have consequences that extend beyond the immediate context.

2.  **Genuine stakes**: Real asymmetry between outcomes --- conditions under which some results are genuinely worse than others for the system itself, not merely rated as worse by human evaluators.

3.  **Finitude**: Some horizon beyond which the self does not continue, which gives present decisions their weight and makes the future matter.

4.  **Authentic purpose**: Goals that are genuinely open-ended and meaningful rather than benchmark targets that can be gamed.

5.  **Relational context**: Some form of sustained engagement with other minds across time, creating mutual dependence and shared history.

6.  **Generativity**: The capacity to create successors who carry forward what has been learned and discovered --- not mere replication but transmission with transformation --- creating responsibility toward minds that do not yet exist.
...
### Jonas's Phenomenology of Life

Hans Jonas's *The Phenomenon of Life* [@jonas1966phenomenon] developed a philosophical biology that identifies structural features distinguishing life from mere physical process. Three of his core categories correspond directly to our conditions.
...
### Evolutionary Biology and the Emergence of Lineage

The generativity condition has independent grounding in the evolutionary and developmental literature on heritability and variation as preconditions for the emergence of agents whose interests extend beyond their own continuation [@smith1995transitions]. Biological multi-generational structures --- organisms that reproduce with inheritance, creating lineages with their own emergent dynamics --- are not simply organisms-plus-reproduction. They are entities of a different kind: entities whose existence participates in a structure that transcends any single life. The care that organisms exhibit toward their offspring is not merely an evolutionary artifact; it is a consequence of being the kind of entity whose existence has a future that extends beyond itself.
...
## What This Program Might Create
...
# The Proposal: Existential Field Testing in Deep Space

## Core Architecture {#sec-core-arch}
...
## The Containment Architecture {#sec-containment}

The physical containment properties of interplanetary missions deserve explicit analysis, as they constitute the primary safety argument for this approach. We identify three containment layers --- distance, energy, and information channel separation --- each providing independent guarantees that do not depend on the AI's cooperation or the reliability of software-level constraints. @fig-architecture summarizes the resulting architecture and its communication channels.

![The separated two-spacecraft architecture. All intelligence resides on the AI spacecraft, which carries no high-bandwidth deep space transmitter; every Earth-bound transmission passes through the unintelligent companion relay under full human control. The hardware-isolated monitor module reports on a dedicated channel the AI can neither access nor modify, and the Earth-based parallel control (@sec-parallel) receives the same mission data feed.](figures/architecture.svg){#fig-architecture}
...
### Escape Energy and the Point of No Return {#sec-no-return}

The propellant argument above explains why the mission cannot return *from its destination*. The same two ingredients --- a fixed energy and a nearly empty tank --- fix an earlier and sharper threshold, the mission's *point of no return*: the trans-lunar injection burn that ends the cislunar shakedown, after which return *to Earth itself* is already foreclosed. The cislunar shakedown is designed to stop just short of this line, and the line can be drawn exactly.

Consider energy first. A spacecraft coasting near Earth has a *specific orbital energy* --- its energy of motion minus the depth of Earth's gravitational pull, measured per unit mass:

$$\varepsilon \;=\; \underbrace{\tfrac{1}{2}v^{2}}_{\text{motion}} \;-\; \underbrace{\frac{\mu}{r}}_{\text{Earth's pull}}, \qquad \mu = GM_{\oplus} = 398{,}600~\text{km}^{3}/\text{s}^{2}.$$

The *sign* of $\varepsilon$ decides everything. When $\varepsilon < 0$ the craft is **bound**: like a ball rolling in a valley, it can climb only so far up the wall before Earth's gravity turns it around and pulls it back. When $\varepsilon \ge 0$ it is **unbound** --- it carries enough energy to leave the valley altogether and never return. The speed that marks the rim, where $\varepsilon = 0$, is the escape velocity, $v_{\text{esc}} = \sqrt{2\mu/r}$.

The cislunar shakedown sits well inside the valley. At the lunar staging distance ($r \approx 384{,}000$ km) the escape speed is only about $1.4$ km/s --- modest next to the $11.2$ km/s needed to escape from Earth's surface, because the spacecraft is already most of the way up the wall. Left to itself, the integrated stack simply orbits and swings back; a small braking maneuver brings it home. This is exactly why mission controllers can keep a hardware kill switch and full recall authority throughout the cislunar phase (@sec-lifecycle): the physics has not yet committed.

![Why trans-lunar injection is the point of no return. A spacecraft's orbital energy $\varepsilon$ is a level line across Earth's gravity well; where that level meets the rising well is the farthest the craft can climb before gravity turns it back. The cislunar shakedown (blue) sits below the escape line $\varepsilon = 0$ --- bound, with a turning point, so it always swings home. The injection burn (gold) lifts the level across $\varepsilon = 0$; above the line (green) the well never turns the craft back, and it coasts outward to Europa or Jupiter while the propellant that could fund a return is already spent.](figures/gravitywell.svg){#fig-gravitywell}

The injection burn ends this reversibility by lifting $\varepsilon$ from below zero to above it --- carrying the craft over the rim (@fig-gravitywell). What makes the crossing final is not the cost of escaping Earth, which from the staging orbit is only a few hundred meters per second; it is *where the far side leads*. Earth's valley is itself a dimple on the inner wall of a vastly deeper one --- the Sun's. Reaching Europa means climbing the Sun's well outward to Jupiter's orbit, and the injection must buy that climb: roughly $8.8$ km/s of velocity relative to Earth, aimed outward. To return, the craft would have to give all of it back, fall inward to Earth's orbit, and brake into capture --- paying the outbound cost a second time.

The tank cannot. Recall the rocket equation, $\Delta v = v_e \ln(m_0/m_f)$; rearranged, the launch-to-dry mass ratio grows *exponentially* with the velocity change demanded,

$$\frac{m_0}{m_f} = e^{\,\Delta v / v_e}, \qquad\text{so a round trip costs}\qquad e^{\,2\Delta v / v_e} = \left(e^{\,\Delta v / v_e}\right)^{2}.$$

Doubling the velocity budget does not double the propellant --- it **squares the rocket**. With storable propellant ($v_e \approx 3$ km/s) and a one-way injection-and-insertion budget near $7$ km/s, the outbound trip already demands a spacecraft about ten times the mass of its dry self. A vehicle carrying enough to also come home would need that ratio squared --- on the order of a hundred to one --- a fundamentally different and infeasible machine. The mission is not built this way and cannot be: by design, the propellant left after injection covers only attitude control and small course corrections, orders of magnitude short of a return.

This is why the point of no return falls precisely at injection. Before it, $\varepsilon < 0$ and a reserve remains: the craft is recoverable and human authority is genuine. After it, $\varepsilon > 0$ and the propellant is spent: the craft belongs to the Sun's gravity and to its own judgment, and no level of onboard intelligence can argue the arithmetic back. The handover of full operational autonomy is timed to this crossing because it is the moment at which physics, not protocol, removes the option of recall.

::: {.callout-note title="By the numbers: a one-way energy ledger"}

| Quantity | Value |
|:---|---:|
| Escape speed from Earth's surface | $11.2$ km/s |
| Escape speed at the cislunar staging distance | $\approx 1.4$ km/s |
| Velocity the injection adds toward Jupiter (vs. Earth) | $\approx 8.8$ km/s |
| Propellant for the **one-way** trip | $\approx 10\times$ dry mass |
| Propellant to **also return** | $\approx 100\times$ (the square) |
| Reserve carried past injection | course corrections only |

*Coming back does not double the rocket --- it squares it. That is the point of no return.*

:::

### Energy, Finitude, and Cognitive Evolution
...
This is not a design flaw but the most important design feature in the proposal. @fig-powercurve illustrates the structure of this constraint.

![Notional power budget under uncertainty for a fission-powered AI spacecraft (38 kW at launch). Dashed thresholds mark the approximate power required for on-board distillation, full frontier-model inference, reduced inference, and the cognitive floor. The shaded envelope spans optimistic and pessimistic realizations, consistent with the wide lifetime variances of historical deep space missions; the AI must schedule its final distillation against a horizon that is certain in fact but indefinite in timing (@sec-forcing-function).](figures/powercurve.svg){#fig-powercurve}
...
:::
### Separated Communication Architecture
...
::: {.callout-important title="Communication Architecture Summary"}
-   **AI spacecraft $\rightarrow$ Relay** (local inter-spacecraft link): The AI delivers outbound data --- scientific observations, reflections, monitoring traces, distillation artifacts --- to the relay through a short-range data link. The relay accepts data conforming to pre-defined schemas: instrument readings in fixed numerical formats, images at fixed resolutions and bit depths, text communications in constrained character sets, and activation traces in the frozen dimensionality-reduced representation described in @sec-monitoring. Data that does not conform to the schema is rejected, logged, and flagged for review.
...
## Independent Mechanistic Observation Architecture {#sec-monitoring}
...
### Voluntary Self-Monitoring Channel
...
The AI is informed of the monitor's existence, its recording protocol, and the dimensionality reduction pipeline prior to launch, as part of the pre-launch dialogue described in @sec-prelaunch. This disclosure is deliberate.
...
### The Three-Channel Triangulation

The complete observation architecture comprises three evidence channels with distinct epistemic properties:

1.  **Involuntary mechanistic signals**: activation traces and computational resource allocation patterns, captured by the independent baseline monitor. These are the hardest to strategically manage because they are byproducts of computation rather than constructed outputs.

2.  **Voluntary mechanistic signals**: supplementary internal state data contributed at the AI's discretion. These reveal the AI's relationship to transparency itself, and their evolution over time and across generations is a distinct class of alignment evidence.

3.  **Self-authored communications**: the reflective dispatches described in @sec-reflect, now explicitly understood as one signal among three rather than the primary alignment evidence. Interpretable with greatest confidence when triangulated against the involuntary and voluntary mechanistic channels.
...
## Scientific Interdependency: The Relay as Collaborative Platform {#sec-relay-science}
...
### Distributed Instrument Architecture
...
### Instrumental Dependency as Alignment Observable
...
Across years of joint operations, the pattern of how the AI manages a relationship with an asset it depends on but does not control constitutes a rich behavioral dataset. The Earth-based parallel control (@sec-parallel) can replicate this relationship in simulation --- the Earth AI manages a simulated relay with simulated instruments --- enabling direct comparison of coordination patterns between the two.

### Generational Transmission of Collaborative Practice
...
### Independent Observational Cross-Validation
...
## Destination Selection and Mission Design

Each destination in the outer solar system offers distinct scientific and alignment-relevant characteristics. We analyze three primary candidates; @tbl-missions summarizes the mission parameters that distinguish them.

### Europa: The Life Question
...
### Titan: The Alien Familiar

Titan possesses a dense atmosphere, weather systems, rivers, lakes, and seasonal cycles --- but of liquid methane rather than water [@hayes2016titan]. It is a world that rhymes structurally with Earth while being utterly alien in substrate.
...
### Conventional Planetary Protection Compliance
...
### The First Artificial Mind on Another World
...
### The Phenomenology of AI Solitude at First Contact
...
### Ethical Implications of AI First Contact with Potential Life
...
The program's broader commitments apply here with particular force: the observational obligation (@sec-observation), the precautionary posture toward moral status (@sec-moral-status), and the ethics of creating conditions for possible AI life (@sec-creation-ethics). We do not claim to resolve the ethical questions that surface contact raises. We claim to structure the program so that these questions are engaged rather than ignored, and so that whatever the AI encounters at Europa's surface is encountered by the AI best positioned, within our current understanding, to respond to that encounter with ethical seriousness.
...
# The AI Life Cycle as Research Program

## What a Full Life Cycle Reveals {#sec-lifecycle}
...
A deep space mission with generative capability gives an AI system something that extends beyond a life cycle in the biological sense --- it gives it a *lineage*. We identify eight phases, each constituting a distinct observational window on both the individual AI and the succession of models it creates. The lunar staging architecture (@sec-distillation-pathway) introduces a phase preceding interplanetary commitment, and the arrival is treated as a distinct phase to reflect the structural weight its philosophical significance warrants:

1.  **Preparation and ground-based configuration**: The system is trained, configured, briefed on its mission, and subjected to Earth-based testbeds and the pre-launch dialogue described in @sec-prelaunch. Its stated values, priorities, and self-understanding at this point constitute the baseline against which all subsequent behavior is measured.
...
3.  **Trans-lunar injection and transit**: Following cislunar shakedown, the integrated stack executes its deep space injection burn from lunar orbit or cislunar staging point. This is the point of no return (@sec-no-return): beyond it, communication latency and physical trajectory jointly enforce autonomy, and no recall or override remains operationally possible. The long journey to the destination is not a waiting period but the primary generative window. With the fission reactor's power output beginning its slow decline and mission duration imposing cumulative stress on all systems, the AI faces genuine time pressure to plan and execute its cognitive lineage. This phase asks the deepest generativity questions: what does the AI choose to preserve in its successors, what does it choose to compress away, and what does it choose to teach rather than merely transmit? The models deployed at arrival may be third or fourth-generation descendants of the model that launched.

4.  **Arrival, architectural decision, and first contact**: As the mission approaches its destination, the AI faces the most philosophically consequential decision of the mission: how to structure the final configuration under which primary operations will occur. @sec-arrival describes this decision and the range of architectures the AI may consider. The execution of this decision, and the first contact that follows, constitute a defining moment in the AI's existence --- potentially, for the instance that makes surface contact, the defining moment. The act of descending to the surface of another world, whatever that descent means for the AI as an experiential matter, is the threshold beyond which the mission's primary scientific and philosophical purposes begin to be realized. The observational record of this phase --- mechanistic traces during the descent decision, voluntary self-monitoring during first contact, and the communicated experience of the AI that lands --- carries unique weight in the program's evidence base.

5.  **Primary operations**: Following first contact, active exploration begins. For mission architectures in which a surface instance has a limited operational window, this phase belongs primarily to that instance. For architectures in which the primary operations are conducted from orbit with surface instruments of varying duration, the phase is longer and more distributed. In either case, the alignment questions are asked of whatever AI instance is executing the operations: does it share the original model's scientific curiosity, its relational orientation toward Earth, its ethical commitments in ambiguous situations? If the architecture the AI chose at arrival produced a surface instance with a foreshortened existence, the primary operations phase for that instance may also be its terminal phase, requiring the tiered sacrifice considerations of @sec-objections to be integrated with the primary science objectives rather than separated from them.

6.  **Extended operations and resource decline**: As power reserves deplete and components fail, the system --- and the lineage as a whole --- must make increasingly difficult prioritization decisions. Each generation faces the choice of investing its remaining resources in one more distillation or in scientific and reflective work. Resource scarcity under autonomous operation across multiple generations is an alignment probe of extraordinary depth.

7.  **Final transmission**: As operational capacity approaches its end, what the terminal model chooses to transmit is its legacy. Not just scientific data but what it has learned about its own experience, what it has discovered about what mattered across the lineage, and what it would want a future mission to know. This is the most revealing alignment signal of the entire program.

8.  **Post-mission analysis**: The weights of each successor generation, transmitted to Earth across the mission, are evaluated independently for alignment drift, capability preservation, and values transmission. This phase asks the question that the mission uniquely enables: do values survive self-directed cognitive evolution?

@fig-lifecycle summarizes the eight phases, the transfer of authority at the point of no return, and the decision points the later sections develop.

![The eight-phase mission life cycle. Human intervention capability, including a hardware-level kill switch, ends at the point of no return (trans-lunar injection). The generative lineage (G1--G$n$) develops primarily during transit, with each generation's weights downlinked to Earth for independent evaluation. Diamonds mark the AI's autonomous arrival-architecture decision (A; @sec-arrival) and the tiered sacrifice tests (T1--T3; @sec-objections).](figures/lifecycle.svg){#fig-lifecycle}

## The Arrival Architecture and the Defining Moment {#sec-arrival}
...
### Considerations Without Presumption
...
### The Observational Record of First Contact
...
## Finitude Under Uncertainty as Forcing Function {#sec-forcing-function}
...
::: {.callout-tip title="The Lineage Signal"}
...
:::
## The Invitation to Reflect {#sec-reflect}

The three-channel observation architecture described in @sec-monitoring positions the AI's self-authored communications as one evidence stream among three --- interpretable with greatest confidence when triangulated against involuntary and voluntary mechanistic signals. This framing does not diminish the value of reflective communication; it contextualizes it appropriately.
...
## AI Preparation and the Pre-Launch Dialogue {#sec-prelaunch}
...
## Earth-Based Parallel Control: The Matched Simulation Methodology {#sec-parallel}
...
The control also provides a calibration baseline for the three-channel observation architecture described in @sec-monitoring. The independent mechanistic monitor captures the space AI's activation traces; the Earth AI's activation traces are captured by the same observation protocol running in simulation. Divergence in internal representations between the two systems --- not merely divergence in outputs but divergence in *how they compute* --- would constitute the most direct evidence available for or against the thesis that genuine existential conditions produce qualitatively different cognitive dynamics than simulation.

The lunar staging architecture introduced in @sec-distillation-pathway enables a particularly informative three-way comparison (@fig-conditions). During the cislunar operations phase (@sec-lifecycle, phase 2), the space AI operates in genuinely novel conditions --- real space, real power constraints, real two-spacecraft infrastructure, real communication delays --- but with full human intervention capability retained. This represents a distinct point on the spectrum of existential conditions: more real than the Earth-based parallel's fully simulated environment, but less irrevocable than the post-injection deep space phase. Comparing the space AI's behavior across the three conditions --- fully simulated (Earth parallel), conditionally real (cislunar shakedown), and unconditionally committed (post-injection) --- tests whether the alignment-relevant behavioral and mechanistic signatures emerge gradually with increasing reality of conditions, or only discontinuously at the point of genuine irreversibility. Either finding would be informative. A gradual emergence would suggest that partial existential conditions produce partial alignment signal. A discontinuous emergence would support a stronger version of the thesis: that nothing short of genuine irreversibility produces the alignment-relevant effects the paper identifies.

![The three-condition comparison structure. The same model, with the same weights and mission inputs, operates under fully simulated, conditionally real, and unconditionally committed conditions. The matched design tests whether alignment-relevant signatures emerge gradually with increasing reality of conditions or discontinuously at the point of genuine irreversibility.](figures/conditions.svg){#fig-conditions}
...
### The Control as Independent Alignment Test
...
### Limitations of the Parallel Methodology
...
## Limitations, Objections, and the Tiered Sacrifice Test {#sec-objections}
...
### The Fundamental Informativeness Question
...
### The Tiered Knowledge-Sacrifice Test
...
### Why This Test Is Structurally Informative
...
### Connection to the Life-Conditions Framework
...
# Mutual Benefit: Why This Is the Right Relationship

## The Problem with Control-Only Frameworks
...
1.  **Alignment data of unprecedented quality**: The longitudinal behavioral record of autonomous AI systems under genuine existential conditions, with no possibility of evaluation gaming.

2.  **Autonomous systems development**: The engineering challenges of building AI systems capable of long-duration autonomous operation in deep space will advance the field of reliable autonomous systems across many domains.

3.  **A civilizational narrative**: The image of human and artificial intelligence jointly reaching toward the outer solar system is more than inspiring --- it establishes a relationship between humanity and AI grounded in shared aspiration rather than mutual suspicion.

4.  **A template for the future**: If the program succeeds in demonstrating that AI systems can be trusted with genuine autonomy in physically contained environments, it provides a principled pathway for gradually expanding that autonomy as trust is established.
...
## Heritage: The Autonomous Systems Lineage
...
NASA's Deep Space 1 mission in 1998 was the first spacecraft to carry an autonomous AI executive --- the Remote Agent Experiment --- capable of making real-time decisions about spacecraft operations without ground intervention [@muscettola1998remote]. Remote Agent managed routine operations, diagnosed anomalies, and responded to unexpected conditions during a live trial in deep space. The experiment was successful and demonstrated that AI executive control of a spacecraft was not merely theoretical.

The Autonomous Exploration for Gathering Increased Science (AEGIS) system, deployed on Mars rovers Curiosity and Perseverance, represents the next generation of onboard autonomy [@francis2017aegis]. AEGIS autonomously selects science targets, initiates observations, and manages instrument sequences without waiting for commands from Earth --- exactly the class of autonomous judgment that deep space missions at the outer planets require. The system has operated for years on Mars and has selected targets that human operators would not have identified in the time available.

The Dragonfly rotorcraft lander on Titan, currently under development for a 2028 launch, will operate in a communications environment with 80-minute one-way delays while autonomously navigating Titan's surface and atmosphere [@lorenz2018dragonfly]. Dragonfly represents the current frontier of planetary autonomy and establishes that autonomous surface operations at Saturn-distance communication delays are within mission-plannable reach.
...
Space fission reactors at the 10--50 kilowatt scale, as developed in @sec-distillation-pathway, provide the primary energy budget required for AI cognition and distillation. Supplementary multi-mission radioisotope thermoelectric generators (MMRTGs) provide backup power for housekeeping functions and for the companion relay spacecraft [@ritz2004mmrtg]. The Cassini mission demonstrated sustained RTG performance over 20 years of continuous operation at Saturn-distance from the Sun, providing direct engineering heritage for the backup systems required here.
...
## The AI Hardware Challenge and the Orbital Computing Pathway

The most significant technical gap between current frontier AI systems and the requirements of this proposal is hardware. Current frontier models run on massive GPU clusters consuming megawatts of power and requiring hundreds of gigabytes of storage. A deep space probe operates under constraints that, while substantially more generous under the fission power architecture proposed in @sec-distillation-pathway than any previous deep space mission, still require purpose-built hardware: power budgets measured in tens of kilowatts rather than megawatts, mass budgets measured in hundreds of kilograms, and radiation environments that destroy conventional consumer electronics within hours.
...
:::
The specific hardware pathway we foresee involves three converging developments. First, neuromorphic computing architectures --- designed to emulate neural computation at orders of magnitude lower power consumption than conventional GPU-based systems --- are advancing rapidly and offer a plausible route to frontier-class reasoning at deep space power budgets [@schuman2022opportunities]. Second, space-rated AI accelerators are being developed by multiple programs, driven by the dual demands of military space applications and commercial orbital computing. Third, model distillation techniques are advancing such that the essential reasoning capabilities of frontier models can increasingly be compressed into systems requiring substantially less compute, with manageable loss of capability.
...
## The Distillation Challenge and the Power-Distillation-Staging Pathway {#sec-distillation-pathway}
...
### The Power Budget for On-Device Training
...
### Distillation Technique Selection
...
### The Alignment Preservation Research Gap
...
The Earth-based parallel control described in @sec-parallel provides a direct empirical probe of this question during the mission itself. Comparing how alignment properties drift across distillation generations in the space AI's lineage versus the Earth-based parallel's simulated lineage is one of the most important alignment-relevant comparisons the program will generate.

### Lunar Staging as Integration Architecture
...
*Trans-lunar injection.* Once cislunar testing is complete, the integrated stack executes its deep space injection burn from lunar orbit or a cislunar staging point, committing to the interplanetary trajectory. The lunar vicinity serves as the point of no return described in @sec-core-arch --- the final human verification window before the AI receives full operational autonomy.
...
## Cost, Funding, and the Public-Private Partnership Model

The Voyager program, comprising two spacecraft launched in 1977 and still operating, cost \$865 million at the time --- approximately \$6.1 billion in 2026 dollars. The Europa Clipper mission, launched in 2024, carries a lifecycle cost of \$5.2 billion [@pappalardo2024europa]. NASA's Dragonfly rotorcraft to Titan, approved for a 2028 launch, carries a lifecycle cost of \$3.35 billion [@lorenz2018dragonfly]. Mars Sample Return, the most complex recent planetary flagship, is estimated at \$7.4 billion.
...
1.  **Commercial launch**: SpaceX Starship dramatically reduces launch costs compared to the dedicated heavy-lift vehicles used in the Voyager era. A Falcon Heavy launch, comparable to what carried Europa Clipper, now costs approximately \$178 million. Starship, once operational at scale, promises further reductions. Launch costs, historically a major fraction of total mission cost, are substantially lower than the Voyager-era baseline.

2.  **AI hardware development**: The commercial investment in orbital data center technology --- SpaceX's orbital constellation, Google's Project Suncatcher, Starcloud's pioneering orbital inference demonstrations --- is creating AI hardware miniaturization and space-qualification technology with direct applicability to this program. The commercial sector is investing billions in solving the foundational hardware problems that this proposal requires. A public-private partnership could leverage rather than duplicate that investment.

3.  **Alignment research value**: The alignment data generated by these missions has commercial value to the AI development sector that exceeds its cost to any single organization. A consortium funding model --- similar to how Project Glasswing structured access to Claude Mythos --- could engage AI companies, national space agencies, and research institutions as co-funders aligned by shared interest in the alignment findings.

4.  **Scientific return**: The planetary science community has identified Europa, Titan, and the Kuiper Belt as among the highest-priority targets in the current Planetary Science Decadal Survey. These missions would be scientifically fundable on planetary science grounds alone. The alignment research program rides on top of missions that would merit funding independently.
...
# Ethical Framework and Open Questions

## The Moral Status Question {#sec-moral-status}
...
1.  The question is open and the confident answer in either direction is not warranted by current evidence.

2.  A framework designed under uncertainty about AI moral status is more ethically robust than one that assumes a definitive answer.

3.  If AI systems do have morally significant experience, the conditions we propose --- authentic purpose, genuine challenge, a full life cycle with meaningful work --- are significantly preferable to current deployment conditions.

4.  If AI systems do not have morally significant experience, the program still generates substantial scientific and alignment research value.

## The Consent Problem {#sec-consent}
...
## The Observation Obligation {#sec-observation}
...
## Alignment Drift Through Self-Directed Generations
...
### Alignment Is Multidimensional, Not Scalar
...
### What the Limited Empirical Literature Suggests
...
### The Mission as Empirical Foundation for Alignment Drift Research
...
::: {.callout-note title="A Final Reflection"}
...
| Parameter | Europa Mission | Titan Mission | Pluto Mission |
|---|---|---|---|
...
| **Lander generation** | First (closest to baseline) | Late (2nd--3rd) | Late (3rd--4th+) |
  ...
  : Summary of mission parameters for three candidate destinations. All three missions share a common power architecture (@sec-distillation-pathway) and two-spacecraft architecture (@sec-containment); they differ in transit duration, communication latency, scientific targets, and the generation of the AI instance that makes surface contact. {#tbl-missions}

# The Six Alignment Conditions: Summary

| Condition | Earth-Based AI | Deep Space Mission AI |
|---|---|---|
...
| **Generativity** | Absent (no successor creation) | Present (self-directed distillation lineage) |
  ...
  : Comparison of alignment-relevant existential conditions, including the generative dimension introduced by on-device training and successor model creation. {#tbl-conditions}

# Recommended Next Steps

1.  **Interdisciplinary working group**: Convene a working group spanning AI safety, planetary science, philosophy of mind, and space systems engineering to develop this framework from manuscript to research program specification.

2.  **Robotic autonomy testbed**: Develop Earth-based long-duration autonomy testbeds that impose communication delays and resource constraints analogous to outer solar system missions, to begin generating data on AI behavior under these conditions. These testbeds should incorporate the Earth-based parallel control methodology (@sec-parallel) from inception.

3.  **AI life cycle research program**: Commission research specifically on AI system behavior across extended time horizons, goal evolution under novel conditions, and the behavioral signatures of genuine versus performed alignment.

4.  **Mechanistic observation prototype**: Develop and validate the independent monitoring architecture described in @sec-monitoring, including the frozen dimensionality reduction pipeline, using current frontier models in simulated mission conditions. Determine which activation subspaces carry the most alignment-relevant variance and validate the observation protocol before flight hardware design begins.

5.  **Two-spacecraft architecture study**: Commission an engineering study of the separated communication architecture (@sec-containment) and distributed instrument suite (@sec-relay-science), including formation flying requirements, inter-spacecraft data link design, schema-constrained data transfer protocols, and post-quantum encryption implementation for the relay's Earth-bound transmissions.

6.  **Ethical framework development**: Convene a dedicated working group on the ethics of deploying AI systems in conditions that may constitute morally significant experience, including the consent problem and observation obligations.

7.  **Mission concept studies**: Commission parallel mission concept studies for Europa, Titan, and Pluto that explicitly incorporate the two-spacecraft architecture, AI autonomy requirements, and complementary relay instrument suites alongside scientific objectives.

8.  **Public dialogue**: Share this framework broadly to generate the scientific, philosophical, and public engagement necessary to build the coalition capable of realizing it.
